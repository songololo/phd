...
This version uses completely customised dijkstra-based algorithms.

The angular version keeps searching (no max distance) until it satisfies all vertices reached by euclidean version

i.e. it is too slow and thus strict max-distance version is probably preferable
...
@jit(nopython=True, nogil=True, parallel=False)
def shortest_path_angular_tree(nbs_arr, ang_dist_arr, dist_arr, source_idx, reachable, active, dist_map_a, dist_map_a_metres, pred_map_a):
    '''
    ...
    dist_map_a_metres[source_idx] = 0
    active[source_idx] = source_idx  # store actual index number instead of booleans, easier for iteration below:

    # search to max distance threshold to determine reachable verts
    while np.any(reachable):
        # sometimes there was an unreachable node, this seems to have been resolved with buffering
        # and only computing centralities for items within city footprint
        # i.e. they have adequate buffering all around to search and find angular paths to reachables
        if not np.any(np.isfinite(active)):
            print('ABORTED')
            print('INFO: no more finite vertices to explore from:')
            print(source_idx)
            print('INFO: number of yet unreached vertices:')
            print(reachable.sum())
            break
        # get the index for the min of currently active vert distances
        # note, this index corresponds only to currently active vertices
        ...
        for n, degrees, meters in zip(nbs_arr[v_idx], ang_dist_arr[v_idx], dist_arr[v_idx]):
            ...
            if not np.isfinite(n):
                break
            n_idx = np.int(n)  # cast to int for indexing
            # set corresponding boolean in reachable to False
            # once all are False all have been reached and while loop exits
            reachable[n_idx] = False
            # check that the neighbour was not directly accessible from previous node
            # this prevents shortcutting
            # first get the previous vertex's id
            prev_nb_idx = pred_map_a[v_idx]
            # need to check for nan in case of first vertex
            if np.isfinite(prev_nb_idx) and np.any(nbs_arr[np.int(prev_nb_idx)] == n_idx):
                # don't proceed with this index if it could've been followed from predecessor
                # i.e. check that it isn't an angular distance backtrack
                continue
            # distance is previous distance plus new distance
            d_a = dist_map_a[v_idx] + degrees
            d_m = dist_map_a_metres[v_idx] + meters
            # only pursue if less than prior assigned distances
            if d_a < dist_map_a[n_idx]:  #  and d_m < max_dist * 1
                dist_map_a[n_idx] = d_a
                dist_map_a_metres[n_idx] = d_m
...
def custom_angular_centralities(nbs, lens, angles, city_ids, max_dist,
    ...
    dist_map_a_metres = np.full(total_count, np.inf)
    pred_map_a = np.full(total_count, np.nan)
    reachable = np.full(total_count, False)

    # iterate through each vert and calculated the centrality path tree
    for source_idx in prange(total_count):

        if source_idx and source_idx % 1000 == 0:
            print('...progress')
            print(round(source_idx / total_count * 100, 2))
        ...
        # prepare arrays
        active.fill(np.nan)
        dist_map_m.fill(np.inf)
        pred_map_m.fill(np.nan)

        shortest_path_tree(nbs, lens, source_idx, max_dist, active, dist_map_m, pred_map_m)

        # prepare arrays
        active.fill(np.nan)
        dist_map_a.fill(np.inf)
        dist_map_a_metres.fill(np.inf)
        pred_map_a.fill(np.nan)
        reachable.fill(False)
        reachable[np.isfinite(dist_map_m)] = True

        # get the angular centrality path
        # done one-by-one otherwise angular tree search doesn't know when to stop
        shortest_path_angular_tree(nbs, angles, lens, source_idx, reachable, active, dist_map_a, dist_map_a_metres, pred_map_a)

        # use the metres distance map to filter reachable verts
        # use corresponding indices
        for to_idx in np.where(np.isfinite(dist_map_m))[0]:

            if to_idx == source_idx:
                continue

            dist_m = dist_map_m[to_idx]
            assert dist_m != np.inf
            ...
            dist_a_metres = dist_map_a_metres[to_idx]
            if dist_a_metres == np.inf:
                print('...NOTE -> no corresponding angular distance in metres, skipping')
                continue
...
async def centrality_angular_custom(db_config, network_schema, nodes_table, links_table, city_pop_id, boundary_schema, boundary_table):
    ...
    for count, r in enumerate(db_data):
        ...
        # only primal edge pairs (i.e. dual vertices) are here split and combined into a new interlinking dual-edges
        # i.e. primal vertices with a single neighbour / primary edge are automatically ignored
...
if __name__ == '__main__':
    ...
    for city_pop_id in range(100, 101):
        ...
        loop.run_until_complete(
            centrality_angular_custom(db_config, network_schema, nodes_table, links_table,
