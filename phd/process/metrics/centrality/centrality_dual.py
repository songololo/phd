...
async def centrality_dual(db_config, nodes_table, links_table, city_pop_id, distances):
    ...
    await db_con.execute(f'''
        ...
        CREATE INDEX IF NOT EXISTS city_pop_idx_{links_table_name_only}_dual ON {links_table}_dual (city_pop_id);
    ...
    logger.info('Calculating centrality paths and centralities for centrality-path heuristics')
    ...
    N.compute_node_centrality(measures=measures)
    logger.info(f'Algo duration: {datetime.timedelta(seconds=time.mktime(time.localtime()) - time.mktime(start))}')

    logger.info('Calculating centrality paths and centralities for simplest-path heuristics')
    start = time.localtime()
    angular_measures = [
        'node_harmonic_angular',
        'node_betweenness_angular']
    N.compute_node_centrality(measures=angular_measures, angular=True)
    logger.info(f'Algo duration: {datetime.timedelta(seconds=time.mktime(time.localtime()) - time.mktime(start))}')
    ...
    for measure in measures:
        ...
        await db_con.execute(f'ALTER TABLE {nodes_table}_dual ADD COLUMN IF NOT EXISTS {c_measure} real[];')
    for ang_measure in angular_measures:
        c_ang_measure = f'c_{ang_measure}'
        await db_con.execute(f'ALTER TABLE {nodes_table}_dual ADD COLUMN IF NOT EXISTS {c_ang_measure} real[];')
