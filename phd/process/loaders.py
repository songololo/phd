...
async def postGIS_to_networkX(db_config, nodes_table, links_table, city_pop_id):
    ...
    """
    # TODO: THIS IS A RELIC?
    # logger.info('Setting ghosted nodes')
    # # have to do manually set ghosted nodes if loading from DB - normally set by decompose
    # for n in tqdm(G.nodes):
    #     deg = nx.degree(G, n)
    #     G.nodes[n]['ghosted'] = (deg == 2)
    """
    return G
