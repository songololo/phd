...
def compute_iter(_n_iters,
    ...
    for i in range(_n_iters):
            ...
            for band_idx in range(_bands):
                ...
                if i != 0 and tier_idx != 0:
                    j_flows = data.singly_constrained(_node_map,
                                                      ...
                                                      _distances,
                                                      _betas,
                                                      i_weights=constrained_flows[i - 1][tier_idx - 1][band_idx] * (1 - _spill_rate),
        ...
        for tier_idx in range(_tiers):
            ...
            off_idx = np.intersect1d(np.where(landuse_capacitance[tier_idx] <= 0), np.where(landuse_state[i][tier_idx] == 1))
            landuse_state[i][tier_idx][off_idx] = 0
            # activate new landuses
            on_idx = np.intersect1d(np.where(landuse_capacitance[tier_idx] >= 1), np.where(landuse_state[i][tier_idx] == 0))
            for on_i in on_idx:
