...
for d in distances:
    df_data_dual_full_clean[f'dual_imp_met_far_{d}'] = df_data_dual_full_clean[f'dual_met_farness_{d}'] / df_data_dual_full_clean[f'dual_node_count_{d}'] ** 2
    df_data_dual_100_clean[f'dual_imp_met_far_{d}'] = df_data_dual_100_clean[f'dual_met_farness_{d}'] / df_data_dual_100_clean[f'dual_node_count_{d}'] ** 2
    df_data_dual_50_clean[f'dual_imp_met_far_{d}'] = df_data_dual_50_clean[f'dual_met_farness_{d}'] / df_data_dual_50_clean[f'dual_node_count_{d}'] ** 2
    df_data_dual_20_clean[f'dual_imp_met_far_{d}'] = df_data_dual_20_clean[f'dual_met_farness_{d}'] / df_data_dual_20_clean[f'dual_node_count_{d}'] ** 2

    df_data_dual_full_clean[f'dual_imp_ang_far_{d}'] = df_data_dual_full_clean[f'dual_ang_farness_{d}'] / df_data_dual_full_clean[f'dual_node_count_{d}'] ** 2
    df_data_dual_100_clean[f'dual_imp_ang_far_{d}'] = df_data_dual_100_clean[f'dual_ang_farness_{d}'] / df_data_dual_100_clean[f'dual_node_count_{d}'] ** 2
    df_data_dual_50_clean[f'dual_imp_ang_far_{d}'] = df_data_dual_50_clean[f'dual_ang_farness_{d}'] / df_data_dual_50_clean[f'dual_node_count_{d}'] ** 2
    df_data_dual_20_clean[f'dual_imp_ang_far_{d}'] = df_data_dual_20_clean[f'dual_ang_farness_{d}'] / df_data_dual_20_clean[f'dual_node_count_{d}'] ** 2
    ...
    for table, table_label in zip(tables, table_labels):
        ...
        for theme, theme_label in zip(themes, theme_labels):
            ...
            if len(data) != start_len:
                logger.warning(f'NOTE -> dropped {round(((start_len - len(data)) / start_len) * 100, 2)}% rows due to non-finite values in target: {target}')
            ...
            for col in cols:
                ...
                if len(data) != start_len:
                    logger.warning(f'NOTE -> dropped {round(((start_len - len(data)) / start_len) * 100, 2)}% rows due to non-finite values in var: {col}')
            ...
            logger.info(f'fitted table: {table_label} and target: {target_label} to r^2 accuracy: {round(acc * 100, 2)} for {theme_label}')
