...
for d in distances:
    df_data_full_clean[f'met_closeness_{d}'] = df_data_full_clean[f'met_node_count_{d}'] ** 2 / df_data_full_clean[f'met_farness_{d}']
    df_data_100_clean[f'met_closeness_{d}'] = df_data_100_clean[f'met_node_count_{d}'] ** 2 / df_data_100_clean[f'met_farness_{d}']
    df_data_50_clean[f'met_closeness_{d}'] = df_data_50_clean[f'met_node_count_{d}'] ** 2 / df_data_50_clean[f'met_farness_{d}']
    df_data_20_clean[f'met_closeness_{d}'] = df_data_20_clean[f'met_node_count_{d}'] ** 2 / df_data_20_clean[f'met_farness_{d}']
...
for target, target_label in zip(targets, target_labels):
    ...
    for table, table_label in zip(tables, table_labels):
        ...
        # insert into dict
        acc = metrics.r2_score(testing_targets, pred_test)
        logger.info(f'fitted table: {table_label} and target: {target_label} to r^2 accuracy: {round(acc * 100, 2)} for closeness')
        ...
        # insert into dict
        acc = metrics.r2_score(testing_targets, pred_test)
        logger.info(f'fitted table: {table_label} and target: {target_label} to r^2 accuracy: {round(acc * 100, 2)} for betweenness -> closeness')
