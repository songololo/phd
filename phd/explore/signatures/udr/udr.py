...
def relative_strength_disentanglement(corr_matrix):
  ...
  score_x = np.nanmean(np.nan_to_num(np.power(np.max(corr_matrix, axis=0), 2) / np.sum(corr_matrix, axis=0), nan=0.0))
  score_y = np.nanmean(np.nan_to_num(np.power(np.max(corr_matrix, axis=1), 2) / np.sum(corr_matrix, axis=1), nan=0.0))
...
def lasso_correlation_matrix(vec1, vec2, random_state=None):
  ...
  """
  assert vec1.shape == vec2.shape
  model = linear_model.Lasso(random_state=random_state, alpha=0.1, tol=0.001, max_iter=10000)  # increased tolerance to reduce warnings
  model.fit(vec1, vec2)
