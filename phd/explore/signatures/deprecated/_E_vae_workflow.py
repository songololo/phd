...
from explore.theme_setup import plot_path, logs_path, weights_path
from explore.theme_setup import process_selected_columns
...
from explore.signatures import D_autoencoder_models
        ...
        for inner_theme in ['simple', 'conv', 'lstm']:
            ...
            for ax_row in ax_rows:
                for ax_n, ax in enumerate(ax_row):
                    ...
                    arr /= maxes
                    arr = np.reshape(arr, (len(selected_columns_labels), len(distances)))
                    plot_funcs.plot_heatmap(ax_row[ax_n], arr, selected_columns_labels, distances, set_labels=True, dark=True)
