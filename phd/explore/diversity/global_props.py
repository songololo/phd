...
axes[0].plot(x_fit_line, powerFunc(x_fit_line, *fp1),
             label=f'non-linear LS curve-fit: $\\alpha={round(fp1[1], 2)}$', alpha=Style.alt_col_hl1_a, color=Style.alt_col_hl1, linestyle='-', linewidth=3)
...
axes[0].plot(x_fit_line, np.exp(slope*np.log(x_fit_line) + intercept),
             label=f'log-log OLS: $\\alpha={round(slope, 2)}$', alpha=Style.alt_col_hl2_a, color=Style.alt_col_hl2, linestyle='-', linewidth=3)
