...
for theme_set, theme_labels, t_meta, p_meta, weighted in zip(theme_sets, theme_label_sets, theme_metas, path_metas, theme_wt):
...
for ax_row, ax in enumerate(axes):
    ...
    ax.set_xlim([100, 1600])
    ax.set_xticks(distances_bandwise)
    ax.set_xlabel(f'Correlation for bandwise & length-normalised {theme_label} mixed-use measures to PCA component {pca_dim + 1}')
    ax.set_ylim([0, 1])
    ax.set_ylabel(r"spearman $\rho$")
...
for ax_row, ax in enumerate(axes):
    ...
    for theme_idx, (theme, label) in enumerate(zip(themes, labels)):
        ...
        axes[ax_row].plot(distances_bandwise, corrs, lw=1, alpha=1, linestyle='-', marker='.', color=f'C{theme_idx}', label=label)
