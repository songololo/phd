...
logger.info(
    f'clustering with hdbscan for min cluster size: {min_cluster_size} and min samples: {min_samples} on {len(selected_cols)} dimensions')
...
phd_util.write_col_data(source_table, cluster_labels, f'hdb_m_{min_cluster_size}_{min_samples}_{len(selected_cols)}dim',
                        'int', ml_data_ldn_clean.id, 'id')
