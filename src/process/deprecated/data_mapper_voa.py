...
async def accessibility_calc(db_config, nodes_table, links_table, boundary_table, data_table, data_id_col, city_pop_id,
                             max_dist, data_where=None, test_mode=False):
    '''
    ...
    if test_mode:
        ...
        data_len, data_ids, total_area, data_x_arr, data_y_arr = await postGIS_to_landuses_dict(db_config, data_table,
                                                                                                data_id_col,
                                                                                                'total_area',
                                                                                                boundary_table,
                                                                                                city_pop_id,
        ...
        _, _, rateable_value, _, _ = await postGIS_to_landuses_dict(db_config, data_table, data_id_col,
                                                                    'adopted_rateable_value', boundary_table,
                                                                    city_pop_id, max_dist=max_dist,
                                                                    data_where=data_where)
    ...
    data_assign_map, data_assign_dist = assign_accessibility_data(network_x_arr, network_y_arr, data_x_arr, data_y_arr,
                                                                  max_dist)
    ...
    betas = [-0.08, -0.04, -0.02666666666666667, -0.02, -0.013333333333333334, -0.01, -0.006666666666666667, -0.005,
             -0.0033333333333333335, -0.0025]
    ...
    for arr_idx, vert_idx in enumerate(verts):
        ...
        reachable, reachable_dist = accessibility_agg(arr_idx, max_dist, nbs, lens, data_classes,
                                                      network_x_arr, network_y_arr, data_x_arr, data_y_arr,
                                                      data_assign_map, data_assign_dist)
        ...
        for dist, beta in zip(distances, betas):
            ...
            cl_unique_trim, cl_counts_trim = mixed_uses.dist_filter(classes_unique, classes_counts, classes_nearest,
                                                                    dist)
