...
async def accessibility_calc(db_config, nodes_table, links_table, boundary_table, data_table, data_id_col,
                             data_class_col, data_target_col, city_pop_id, max_dist, data_where=None, test_mode=False):
    ...
    data_len, data_ids, data_classes, data_x_arr, data_y_arr = \
        await postGIS_to_landuses_dict(db_config, data_table, data_id_col, data_class_col, boundary_table, city_pop_id,
                                       max_dist=max_dist, data_where=data_where)
    ...
    for arr_idx, vert_idx in enumerate(verts):
        ...
        reachable_classes, reachable_classes_dist = accessibility_agg(arr_idx, max_dist, nbs, lens, data_classes,
                                                                      network_x_arr, network_y_arr, data_x_arr,
                                                                      data_y_arr, data_assign_map, data_assign_dist)
