...
class Graph_Analyse:
    ...
    def analyse_graph(self, max_distance: int, angular=False):
        ...
        for i_v in self.graph_dual.vertices():
            ...
            if count % update_interval == 0 and count != 0:
                ...
                time_remaining = round(
                    (time_elapsed * ((self.graph_dual.num_vertices() - count) / update_interval) / 60), 2)
                logger.info(
                    '...presently at index {0}, last batch: {1}s, ~ time remaining: {2}m'.format(count, time_elapsed,
                                                                                                 time_remaining))
        ...
        logger.info('...completed graph analysis for distance: {0}, time elapsed: {1}m'.format(max_distance, round(
            (time.time() - time_start) / 60, 2)))
    ...
    def build_dual_graph_links(self, A_v, B_v):
        ...
        for neighbour_vertex in B_v.all_neighbours():
            ...
            if neighbour_vertex != A_v:
                ...
                distance = (self.gd_vert_primary_distance_float[vert_dual] / 2) + (
                            self.gd_vert_primary_distance_float[vert_neighbour] / 2)
...
class Graph_Analyse_PostGIS(Graph_Analyse):
    ...
    async def build_graph(self, dsn_string: str, source_schema: str, source_table: str, wgs_bounds: tuple = None,
                          id_column: str = 'id'):
        '''
    ...
    async def return_graph(self, postfix):
        ...
        closen_weighted = np.where(np.isnan(self.gd_vert_closeness_weighted_float.a), None,
                                   self.gd_vert_closeness_weighted_float.a)
        ...
        between_weighted = np.where(np.isnan(self.gd_vert_betw_weighted_float.a), None,
                                    self.gd_vert_betw_weighted_float.a)
