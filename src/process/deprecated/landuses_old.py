...
def aggregate_uses(verts, class_code_arr, betas, uses_unique, uses_count):
    ...
    for v_idx in verts:
        ...
        accommodation += np.sum(np.isfinite(class_code_arr[v_idx]
                                            [(class_code_arr[v_idx] >= 1010000) & (class_code_arr[v_idx] < 1020000)])) * \
                         betas[v_idx]
        eating += np.sum(np.isfinite(class_code_arr[v_idx]
                                     [(class_code_arr[v_idx] >= 1020000) & (class_code_arr[v_idx] < 2000000)])) * betas[
                      v_idx]
        commercial += np.sum(np.isfinite(class_code_arr[v_idx]
                                         [(class_code_arr[v_idx] >= 2000000) & (class_code_arr[v_idx] < 3000000)])) * \
                      betas[v_idx]
        attractions += np.sum(np.isfinite(class_code_arr[v_idx]
                                          [(class_code_arr[v_idx] >= 3000000) & (class_code_arr[v_idx] < 4000000)])) * \
                       betas[v_idx]
        entertainment += np.sum(np.isfinite(class_code_arr[v_idx]
                                            [(class_code_arr[v_idx] >= 4000000) & (class_code_arr[v_idx] < 5000000)])) * \
                         betas[v_idx]
        manufacturing += np.sum(np.isfinite(class_code_arr[v_idx]
                                            [(class_code_arr[v_idx] >= 7000000) & (class_code_arr[v_idx] < 9000000)])) * \
                         betas[v_idx]
        retail += np.sum(np.isfinite(class_code_arr[v_idx]
                                     [(class_code_arr[v_idx] >= 9000000) & (class_code_arr[v_idx] < 10000000)])) * \
                  betas[v_idx]
        ...
        property += np.sum(np.isfinite(class_code_arr[v_idx]
                                       [(class_code_arr[v_idx] == 2110192) | (class_code_arr[v_idx] == 2110190)])) * \
                    betas[v_idx]
        health += np.sum(np.isfinite(class_code_arr[v_idx]
                                     [(class_code_arr[v_idx] - (class_code_arr[v_idx] % 10000) == 5260000) |
                                      ...
                                      (class_code_arr[v_idx] - (class_code_arr[v_idx] % 10000) == 5290000)])) * betas[
                      v_idx]
        education += np.sum(np.isfinite(class_code_arr[v_idx]
                                        [(class_code_arr[v_idx] - (class_code_arr[v_idx] % 10000) == 5270000) |
                                         ...
                                         (class_code_arr[v_idx] - (class_code_arr[v_idx] % 10000) == 5320000)])) * \
                     betas[v_idx]
        parks += np.sum(np.isfinite(class_code_arr[v_idx]
                                    [(class_code_arr[v_idx] - (class_code_arr[v_idx] % 10000) == 3180000)])) * betas[
                     v_idx]
        cultural += np.sum(np.isfinite(class_code_arr[v_idx]
                                       [(class_code_arr[v_idx] - (class_code_arr[v_idx] % 10000) == 3170000)])) * betas[
                        v_idx]
        sports += np.sum(np.isfinite(class_code_arr[v_idx]
                                     [(class_code_arr[v_idx] - (class_code_arr[v_idx] % 10000) == 4240000)])) * betas[
                      v_idx]
...
async def aggregator(db_config, networks_table, verts_table, links_table, city_pop_id, threshold_distances):
    ...
    g_vert_list_bldgs = graph.new_vertex_property('float')  # should be float because count is weighted
    g_vert_voa_count = graph.new_vertex_property('float')  # should be float because count is weighted
    ...
    if graph.num_vertices():
        ...
        # these are likely due to lumped VOA locations located by postcode or at city centre-point failing more precise info?
        # or are these major shopping malls... ?
        ...
        async with db_con.transaction():
            async for record in db_con.cursor(f'''
                ...
                if record['class_code_arr']:
                    ...
                    arr = np.pad(arr, (0, class_size_cap - len(arr)), mode='constant',
                                 constant_values=0)  # np.nan doesn't work with int
                ...
                if record['voa_area_arr']:
                    ...
                    if len(arr) > max_cap:
                        logger.error(
                            f'Maximum array size of {max_cap} has been exceeded for VOA, skipping this node due to likely outlier')
                        continue
        ...
        for threshold_distance in threshold_distances:
            ...
            for v in graph.vertices():
                ...
                # distinguish between averaging (IDW) and aggregating (gravity)
                # aggregate weighted heritage
                heritage = float(np.nansum(g_vert_list_bldgs.a[verts] * beta_weighted_distances[verts]))
                # aggregated weighted voa counts
                voa_count = float(np.nansum(g_vert_voa_count.a[verts] * beta_weighted_distances[verts]))

                # process valuation data
                voa_area_mean, voa_val_mean, voa_val_cov, voa_rate_mean, voa_rate_cov = \
                    derive_voa(verts, voa_area_arr, voa_val_arr, voa_rate_arr, beta_weighted_distances)
                ...
                mixed_uses_primary_score = stirling_div(uses_unique, uses_probs, beta=1, weight_1=0, weight_2=0,
                                                        weight_3=1)
                mixed_uses_secondary_score = stirling_div(uses_unique, uses_probs, beta=1, weight_1=0, weight_2=1,
                                                          weight_3=0)
                mixed_uses_tertiary_score = stirling_div(uses_unique, uses_probs, beta=1, weight_1=1, weight_2=0,
                                                         weight_3=0)
            ...
            await db_con.execute(f'''
                    ALTER TABLE {networks_table}.{verts_table}
                        ADD COLUMN IF NOT EXISTS listed_bldgs_count_{append} real,
                        ADD COLUMN IF NOT EXISTS voa_count_{append} real,
                        ADD COLUMN IF NOT EXISTS voa_area_mean_{append} real,
                        ADD COLUMN IF NOT EXISTS voa_val_mean_{append} real,
                        ADD COLUMN IF NOT EXISTS voa_cov_val_{append} real,
                        ADD COLUMN IF NOT EXISTS voa_rate_mean_{append} real,
                        ADD COLUMN IF NOT EXISTS voa_cov_rate_{append} real,
            ...
            await db_con.executemany(f'''
                UPDATE {networks_table}.{verts_table}
                    SET listed_bldgs_count_{append} = $1,
                        voa_count_{append} = $2,
                        voa_area_mean_{append} = $3,
                        voa_val_mean_{append} = $4,
                        voa_cov_val_{append} = $5,
                        voa_rate_mean_{append} = $6,
                        voa_cov_rate_{append} = $7,
                        uses_accommodation_{append} = $8,
                        uses_eating_{append} = $9,
                        uses_commercial_{append} = $10,
                        uses_attractions_{append} = $11,
                        uses_entertainment_{append} = $12,
                        uses_manufacturing_{append} = $13,
                        uses_retail_{append} = $14,
                        uses_transport_{append} = $15,
                        uses_property_{append} = $16,
                        uses_health_{append} = $17,
                        uses_education_{append} = $18,
                        uses_parks_{append} = $19,
                        uses_cultural_{append} = $20,
                        uses_sports_{append} = $21,
                        mixed_uses_score_0_{append} = $22,
                        mixed_uses_score_5_{append} = $23,
                        mixed_uses_score_10_{append} = $24,
                        uses_score_primary_{append} = $25,
                        uses_score_secondary_{append} = $26,
                        uses_score_tertiary_{append} = $27,
                        mixed_uses_score_hill_0_{append} = $28,
                        mixed_uses_score_hill_10_{append} = $29,
                        mixed_uses_score_hill_20_{append} = $30,
                        mixed_uses_d_simpson_index_{append} = $31,
                        mixed_uses_d_species_distinctness_{append} = $32,
                        mixed_uses_d_balance_factor_{append} = $33,
                        mixed_uses_d_combined_{append} = $34
                    WHERE id = $35
            ''', results)
...
if __name__ == '__main__':
    ...
    verts_table = 'roadnodes_50'
    links_table = 'roadlinks_50'
    for city_pop_id in range(1, 2):
        logger.info(
            f'Starting network aggregation of data for city id: {city_pop_id} on table {network_schema}.{verts_table}')
        loop.run_until_complete(
